...
$sql="select sum(km) 'total' from timecard_vehicle where vehicle_id='".$vehicle_id."'";
$res=mysql_query($sql);
while($row=mysql_fetch_array($res))
{	
	$end_km=$row['total']; 
...
$start_km=number_format($start_km, 2, '.', '');
$end_km=number_format($end_km, 2, '.', '');
...
<div class="table">
    <h4>Starting Odometer - '.$start_km.'    Ending Odometer - '.$end_km.' </h4>
        <table> 
            <thead>
                <tr>
                    <th>Date</th>
            ...
            <tbody>';
                ...
                $sql="select * from timecard_vehicle where vehicle_id='".$vehicle_id."' and cr_date between '".$start."' and '".$end."'";
				$res=mysql_query($sql);
				while($row=mysql_fetch_array($res))
				{	
					$cr_date=$row['cr_date'];
