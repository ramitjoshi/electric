...
function get_vehicle_num_info($vehicle_num) 
{
	$sql="select * from vehicle where vehicle_num='".$vehicle_num."'";
	$res=mysql_query($sql);  
	$count=mysql_num_rows($res);
	if($count > 0)
	{	
...
function get_vehicle_info($vehicle_id,$field) 
{
	$sql="select ".$field." from vehicle where id='".$vehicle_id."'";
	$res=mysql_query($sql);
	$count=mysql_num_rows($res);
	if($count > 0)
	{	
...
function get_timecard_vehicle_info($card_id,$field) 
{
	$sql="select ".$field." from timecard_vehicle where card_id='".$card_id."'";
	$res=mysql_query($sql);
	$count=mysql_num_rows($res);
	while($row=mysql_fetch_array($res))
	{
...
function get_timecard_info($card_id,$field)
{
	$sql="SELECT $field FROM  `timecard` WHERE  `id` ='".$card_id."'";
	$res=mysql_query($sql);
	while($row=mysql_fetch_array($res))
	{
		return $row[$field];
...
function total_work_by_vehicle($job_id)
{
	$sql="SELECT *
	FROM  `timecard`  
	WHERE job_id =  '".$job_id."'"; 
	...
	{	
		...
		$sql_1="SELECT SUM( km ) 'total_km'
		FROM  `timecard_vehicle` 
		WHERE card_id
		IN (".$job_idd.") 
		GROUP BY card_id";
		$res1=mysql_query($sql_1); 
